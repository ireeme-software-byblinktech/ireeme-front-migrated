 ...
 * Assumes an endpoint `/api/v1/attendance/class/:classId?date=YYYY-MM-DD` exists.
 */
export const getAttendanceByClass = (classId: string, date: string) =>
  apiClient<AttendanceRecord[]>(`/api/v1/attendance/class/${classId}?date=${date}`);
...
export const markBulkAttendance = (
  ...
  apiClient<void>("/api/v1/attendance/mark-bulk", {
    method: "POST",
