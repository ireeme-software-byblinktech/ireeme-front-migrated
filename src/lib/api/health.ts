...
export const healthApi = {
  ...
  closeMedicalCase: (id: string) =>
    apiClient<MedicalCase>(`/api/v1/health/medical-cases/${id}/close`, {
      method: "PATCH",
    }),

  // Appointments
  createAppointment: (data: CreateAppointmentDto) =>
    apiClient<Appointment>("/api/v1/health/appointments", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  updateAppointmentStatus: (id: string, status: AppointmentStatus) =>
    apiClient<Appointment>(`/api/v1/health/appointments/${id}/status?status=${status}`, {
      method: "PATCH",
    }),
};
